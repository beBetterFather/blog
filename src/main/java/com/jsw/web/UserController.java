...
import com.jsw.entity.Trolemenu;
import com.jsw.entity.Tuser;
import com.jsw.service.TmenuService;
import com.jsw.service.TroleService;
import com.jsw.service.TrolemenuService;
...
public class UserController {
    ...
    /**
     * 登录后获取用户的有权限菜单，并保存角色菜单信息到session缓存中
     * shiro 的sessiondao使用的是redis所以理论上应该可以分布式
     * @param roleId
     * @param session
     * @return
     * @throws Exception
...
//        putTmenuOneClassListIntoSession(session);
        putTmenuIntoSessionWithRole(session);
        map.put("success", true);
        return map;
    }
...
//		logService.save(new Log(Log.LOGOUT_ACTION,"用户注销"));
    ...
    public String loadMenuInfo(HttpSession session, Integer parentId) throws Exception {
        ...
        Trole currentRole = (Trole) JsonMapper.fromJsonString(JsonMapper.toJsonString(session.getAttribute("currentRole")), Trole.class);
	...
	public void putTmenuIntoSessionWithRole(HttpSession session){
    	//1、获取登录用户的角色信息
		Trole currentRole = (Trole) JsonMapper.fromJsonString(JsonMapper.toJsonString(session.getAttribute("currentRole")), Trole.class);

		//2、根据角色获取对应的菜单
		Example example=new Example(Trolemenu.class);
		example.or().andEqualTo("roleId",currentRole.getId());
		List<Trolemenu> trolemenus = trolemenuService.selectByExample(example);

		//3、过滤掉非一级菜单，只保留一级菜单用来在welcome.ftl中获取主菜单列表
		Example titleMenus =new Example(Tmenu.class);
        titleMenus.or().andIn("id", trolemenus.stream().map(t-> t.getMenuId()).collect(Collectors.toList()))
                .andEqualTo("pId", 1);
		List<Tmenu> tmenuOneClassList=tmenuService.selectByExample(titleMenus);
		session.setAttribute("tmenuOneClassList", tmenuOneClassList);
	}
